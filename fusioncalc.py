# PokéRogue Fusion Calculator — build 2026-02-22
# - FIX: recalc_if_ready() included (and defined before UI binds to it)
# - FIX: Swap refreshes both side panels, repopulates Active Ability options from new Pokémon 2, and recalculates
# - Options row (center, under buttons): Active Ability combo + Passive checkbox (centered)
# - Tooltips: quick inline help on ability combobox/label and Passive checkbox
# - Additional ability rules: Earth Eater, Well-Baked Body, Heatproof, Water Bubble (+ existing)
# - Mini status bar: fused type, active/passive state, calc time
# - Performance: debounced search filtering
# - No fused name UI (center caption is static). Hidden Ability = 2nd listed ability in CSV.
...
def open_pokemondb():
    webbrowser.open("https://pokemondb.net/")

def open_type_calculator():
    webbrowser.open("https://www.pkmn.help/defense/")

def open_pokedex():
    webbrowser.open("https://ydarissep.github.io/PokeRogue-Pokedex/")
...
ABILITY_EFFECTS = {
    ...
    'SAPSIPPER':        {'immunities': ['Grass']},  # alias safety
...
def calculate_type_effectiveness(type1, type2=None, active_ability=None, passive_ability=None):
    ...
    # Wonder Guard: immune to all non-super-effective moves
...
def calculate_fusion_stats(p1, p2):
    ...
    try:
        if p1 in pokemon_stats and p2 in pokemon_stats:
            ...
            fused_type2 = (
                pokemon_stats[p2]['Type_2']
                if pokemon_stats[p2]['Type_2'] != "" and pokemon_stats[p2]['Type_2'] != pokemon_stats[p1]['Type_1']
            ...
            abilities = list(dict.fromkeys(pokemon_stats[p2]['Abilities']))  # from Pokémon 2
            ...
            passive_ability = pokemon_stats[p1]['Passive']  # from Pokémon 1
...
def recalc_if_ready():
    """Recalculate fusion if both sides are selected."""
...
def on_select(event, pokemon_var, pokemon_name, pokemon_info, pokemon_id):
    ...
    # Fill the appropriate side panel
    fill_side_panel(selected_pokemon, pokemon_info, pokemon_id, pokemon_name)

    # If Pokémon 2 changed, repopulate Active Ability choices and recalc
...
def swap_pokemon():
    """Swap the two selections and fully refresh: panels, ability list, and fused calc."""
    p1 = pokemon1_var.get().strip()
    p2 = pokemon2_var.get().strip()
    pokemon1_var.set(p2)
    pokemon2_var.set(p1)

    # Refresh left/right panels
    ...
    # Repopulate Active Ability list from NEW Pokémon 2 (now p1)
    ...
    try:
        def pos(widget):
            try:
                gi = widget.grid_info()
                if gi:
                    return f"grid r={gi.get('row')} c={gi.get('column')} span={gi.get('columnspan','1')} sticky={gi.get('sticky','')}"
                else:
                    return "(not gridded)"
            except Exception:
                return "(n/a)"
        ...
        lines = []
        lines.append(f"Active Ability combobox present: {bool(active_ability_combo)}  @ {pos(active_ability_combo)}")
        lines.append(f"Passive Active checkbox present: {bool(passive_check)}      @ {pos(passive_check)}")
        lines.append("")
...
def debug_show_row(which: str):
    name = pokemon1_var.get().strip() if which == "p1" else pokemon2_var.get().strip()
    if name not in pokemon_stats:
        messagebox.showwarning("Current Row", f"Selected name not found: {name or '(empty)'}")
        return
    row = pokemon_stats[name]
    msg = (
        f"Name: {name}\n"
        f"ID: {row.get('ID')}\n"
        f"Types: {row.get('Type_1')}{('/' + row.get('Type_2')) if row.get('Type_2') else ''}\n"
        f"Abilities: {', '.join(row.get('Abilities', []))}\n"
        f"Passive: {row.get('Passive')}\n"
        f"BST: {row.get('BST')}\n"
        f"Evolution line: {row.get('evolution line')}\n"
    )
    title = "Current Row — Pokémon 1" if which == "p1" else "Current Row — Pokémon 2"
            ...
            def dump(parent, indent=0):
                for w in parent.winfo_children():
                    ...
                    if manager == 'grid':
                        ...
                        info = {}
                    f.write("  " * indent + f"{w.__class__.__name__} {str(w)} : {manager} {info}\n")
                    dump(w, indent+1)
            dump(root)
        messagebox.showinfo("Debug", "Widget positions written to widget_positions.log")
    except Exception as e:
        messagebox.showerror("Debug", f"Error logging widget positions: {str(e)}")
...
def debug_reload_csv():
    try:
        ...
        messagebox.showinfo("Reload CSV", f"Reloaded pokemon_data.csv\nLoaded: {count} rows")
    except Exception as e:
        messagebox.showerror("Reload CSV", str(e))

debug_menu = tk.Menu(menubar, tearoff=0); menubar.add_cascade(label="Debug", menu=debug_menu)
...
# ---- Layout frame ----
main_frame = tk.Frame(root); main_frame.pack(fill=tk.BOTH, expand=True, padx=10)
...
# Buttons (center)
button_frame = tk.Frame(main_frame); button_frame.grid(row=1, column=2)
...
swap_button = tk.Button(button_frame, text="Swap", command=swap_pokemon); swap_button.pack(side=tk.LEFT, padx=5)

def clear_selections():
    # Clear left/right selected names
    pokemon1_var.set(""); pokemon2_var.set("")
    ...
    # Clear side panel labels and text
    pokemon1_name.config(text=""); pokemon2_name.config(text="")
    pokemon1_info.delete('1.0', tk.END); pokemon2_info.delete('1.0', tk.END)
    fusion_info.delete('1.0', tk.END)
    pokemon1_id.config(text=""); pokemon2_id.config(text="")

    # Repopulate both filtered lists from full dataset
    pokemon1_filtered_listbox.delete(0, tk.END); pokemon2_filtered_listbox.delete(0, tk.END)
    for name in pokemon_stats:
        pokemon1_filtered_listbox.insert(tk.END, name)
        pokemon2_filtered_listbox.insert(tk.END, name)

    # --- NEW: also clear the Active Ability list + current selection ---
    try:
        active_ability_combo['values'] = [""]   # visually empties the dropdown list
        active_ability_var.set("")              # clears the selected value
    except Exception:
        pass

    # Optional: reset mini status bar
    try:
        status_text.set("Ready.")
    except Exception:
        pass

clear_button = tk.Button(button_frame, text="Clear", command=clear_selections); clear_button.pack(side=tk.LEFT, padx=5)

# ---- OPTIONS ROW (center, below buttons) ----
...
# Row 1: passive checkbox BELOW and CENTERED
...
# Tooltips
class _TT(Tooltip): pass
_TT(active_ability_label, "Main ability comes from Pokémon 2 (the Sacrifice).")
_TT(active_ability_combo, "Pick the Active Ability from Pokémon 2.\nHidden Ability = 2nd listed ability in the dex data.")
_TT(passive_check, "Toggle applying Pokémon 1's Passive to defenses (e.g., Thick Fat halves Fire/Ice).")

# Reactive calc on change
...
# Center result (static caption; no fused name)
...
def apply_dark():
    bg = "#2b2b2b"; fg = "#ffffff"; entry_bg = "#3c3f41"; btn_bg = "#4e5254"; btn_fg = "#ffffff"
    root.configure(bg=bg); main_frame.configure(bg=bg)
    for w in main_frame.winfo_children():
        if isinstance(w, tk.Label): w.configure(bg=bg, fg=fg)
        elif isinstance(w, tk.Button): w.configure(bg=btn_bg, fg=btn_fg)
        elif isinstance(w, tk.Listbox): w.configure(bg=entry_bg, fg=fg)
        elif isinstance(w, tk.Text): w.configure(bg=entry_bg, fg=fg)
        elif isinstance(w, tk.Frame):
            w.configure(bg=bg)
            for c in w.winfo_children():
                if isinstance(c, tk.Button): c.configure(bg=btn_bg, fg=btn_fg)
...
def apply_light():
    ...
    for w in main_frame.winfo_children():
        ...
        elif isinstance(w, tk.Listbox): w.configure(bg=bg, fg=fg)
        elif isinstance(w, tk.Text): w.configure(bg=bg, fg=fg)
        elif isinstance(w, tk.Frame):
            w.configure(bg=bg)
            for c in w.winfo_children():
                if isinstance(c, tk.Button): c.configure(bg=bg, fg=fg)
